...
public class JwtUtil {
    ...
    public int isValid(String token, Integer flag){
        ...
        if ( token == null && flag == 1 ) { // refresh token이 null로 요청된 경우
            throw new RefreshTokenNullException(ErrorMessage.TOKEN_NULL);
        }
        else if ( token == null && flag == 0 ){// access token이 null로 요청된 경우
            throw new AccessTokenNullException(ErrorMessage.TOKEN_NULL);
        }
        else if ( token.equals("Bearer") && flag == 0  ) { // access token이 비어있는 경우 ( "Bearer " 만 온 경우 뒤에 공백이 무시되는 현상 )
            throw new AccessTokenNullException(ErrorMessage.TOKEN_NULL);
        }
        else if ( token.equals("Bearer") && flag == 1){ // refresh token이 비어있는 경우 ( "Bearer " 만 온 경우 뒤에 공백이 무시되는 현상 )
            throw new RefreshTokenNullException(ErrorMessage.TOKEN_NULL);
        }
    ...
    public Map<String, Object> validateFormat(String token,Integer flag) {
        ...
        if ( (token == null || token.length() <= 0  ) && flag == 0 ){
            throw new AccessTokenNullException(ErrorMessage.TOKEN_NULL);
        } else if ( (token == null || token.length() <= 0  ) && flag == 1) {
            throw new RefreshTokenNullException(ErrorMessage.TOKEN_NULL);
        }
